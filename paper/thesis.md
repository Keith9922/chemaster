...
> **版本注记（2026-07-11）**：本文件是已答辩定稿（2026-05-21 答辩，05-30
> 会后修订）的仓库存档，正文数字保持答辩时口径不再更新。答辩后仓库的最新
> 指标（如路由 100%、N=10000 稳定性、故障处置双口径等）以 `README.md` 与
> `benchmarks/engineering_metrics/*.json` 为准。
...
工程层面（按导师反馈把重点从"计算准确率"转向"应答率 + 执行稳定性"）：完成了 3 项指标的实测——(i) 操作性故障自动恢复率 84%（25 次故障注入），(ii) **应答率与工具调用正确性 98.0%**（5 类任务、100 条中英文双语自然语言表述驱动），(iii) **大规模调用稳定性**（N=2000 重复同一 anchor 任务，**100% 成功率、1 个唯一工具调用序列、wall-clock 均值 131 ms、标准差 4.9 ms、p99 = 150 ms，散点+滚动均值无漂移**）。其余三项（提交摩擦时间节省、化学决策推荐接受率、运行轨迹自主步占比）因依赖真人被试或真实大模型 API 而未在本工作中完成数据采集，相关协议已写入文档供后续工作执行。此外，针对导师反馈"大模型领域盲区与研究者个性化偏好"的问题，本工作新增**个人知识库（user_kb）机制**——研究者可在 `~/.chemaster/user_kb/` 上传自定义 skill / 规则 / 偏好（如"SOC 一律用 BDF"、"光谱用 MOMAP"、特定分子家族的方法选择），系统启动时自动合并加载并影响 Agent 的推荐决策，由约 350 行代码 + 25 个新单元测试覆盖（仓库总测试数 281，278 通过 + 3 因测试机器未配置 API key 而跳过）。本工作完成了系统的整体设计、跨多个公开基准的实跑验证、以及大模型 Agent 在工具调度、错误恢复、大规模调用稳定性方面的工程能力评估，并诚实标注了所有数据点的真实性。
...
Validation uses three public benchmarks spanning three task types: the S22 weakly-interacting dimer set (ground-state energetics), the QUEST excited-state reference database (vertical excitation energies, TDDFT), and anthracene (radiative rates and dynamics). Because Gaussian, BDF and MOMAP licenses were not available on the test machine, the real runs reported here were carried out with the open-source program psi4 1.10 (S22 and QUEST); the full anthracene BDF + MOMAP pipeline is left as future work. To compensate for BDF unavailability on the SOC side, this work adds a `chem.calc_pyscf` MCP server (open-source PySCF 2.13, native macOS arm64 support) and runs a real three-stage X2C relativistic calculation on anthracene (non-relativistic → scalar relativistic → two-component including SOC), demonstrating that ChemMaster is backend-agnostic on SOC tasks and that the BDF wrapper has a working open-source reference implementation. At the chemistry level (basic accuracy — limited by the open-source backend's methodology, and intended to demonstrate that the system drives each class of calculation correctly): the full 22-system S22 set (B3LYP-D3(BJ)/def2-TZVP with counterpoise) gives mean absolute errors within the literature range of B3LYP-D3 on this dataset, with canonical-geometry systems such as water_dimer, ammonia_dimer, formic_acid_dimer and ethene_ethyne all below 1 kcal/mol (ethene_ethyne at 0.04 kcal/mol); QUEST over 10 molecules and 20 excited states (TD-CAM-B3LYP/def2-SVP, TDA) gives an overall MAE of 0.64 eV, with the lowest n→π* and π→π* valence states of HCHO, pyridine, acetaldehyde, butadiene and cyclopentadiene all within 0.2 eV — consistent with TD-CAM-B3LYP at this basis (Rydberg states are penalised by the lack of diffuse functions); the anthracene X2C-1e SOC run (PySCF, def2-svp, B3LYP) yields a scalar relativistic correction of −5.28 eV and a sub-meV SOC correction, matching the chemistry of a pure C/H system. At the engineering level (re-focused, following advisor feedback, from "computational accuracy" to "response rate + execution stability"), three indicators were measured: (i) technical fault auto-recovery rate of 84% over 25 fault injections; (ii) response rate and tool-routing correctness of 98.0% over 100 bilingual natural-language phrasings spanning 5 task classes; (iii) large-scale invocation stability over N = 2000 repetitions of the same anchor task — 100% success, a single unique tool-call sequence, mean wall-clock 131 ms (σ = 4.9 ms, p99 = 150 ms) with no drift. The remaining indicators (submission-friction time savings, chemistry-recommendation acceptance rate, trajectory autonomous-step ratio) require human subjects or a live LLM API key and are not collected in this work; their protocols are documented in BENCHMARK_PROTOCOL.md for follow-up. In addition, a personal knowledge base (user_kb) mechanism lets researchers place custom skills, rules and preferences under `~/.chemaster/user_kb/` that are merged at startup and shape the agent's recommendations (about 350 lines of code and 25 new unit tests; 281 tests in total, 278 passing and 3 skipped for lack of an API key on the test machine). The data points reported here honestly reflect what was measured on the test machine and what remains as future work, while validating the system's basic computational capability and the soundness of its labor-saving collaborator architecture.
...
**关于"贡献"的边界声明**：ChemMaster 作为**系统**，其职责是**翻译**——把自然语言意图转化为对量子化学软件的标准化调用；具体的电子结构能量、激发态、SOC 等数值结果由 psi4 / Gaussian / BDF / MOMAP 等化学**软件**决定。因此本工作的可比较贡献天然分为两类：**系统层贡献**（系统自己负责的能力指标，与后端化学软件无关）与**后端兼容性旁证**（在公开 benchmark 上把后端跑通、误差落在所选方法的内禀范围内，说明系统没有把后端"用坏"）。本节列出的 6 项贡献均属系统层；§4.2、§4.3 列出的化学误差则属后端兼容性旁证——任何不调用 ChemMaster、用同样方法+基组的脚本，得到的精度数字都不会变。本工作的主要贡献包括：
...
2. **架构与协议层面**：以 MCP（Model Context Protocol）作为统一的工具抽象协议，将 Gaussian、BDF、MOMAP 等计算后端封装为标准化的 MCP 服务（共 15 个 server）。**整个 Agent 内核也以 MCP server 形态对外开放**（`chemaster.mcp.agent.server`），Claude Code / Cursor / OpenAI Codex CLI 等任意兼容 MCP 的客户端均可挂载并通过 `chemaster_run(intent)` 调用整套化学工作流——交付的不仅是单一 Agent 程序，也是一个可被任意 LLM 客户端复用的化学内核。

3. **多前端实现**：CLI、Textual TUI 与本地 Web 三个前端共享同一 Agent 内核与工具集，覆盖从 SSH 远程开发到浏览器交互的多种使用场景。一行 `pipx` 安装即可（`scripts/install.sh`），并配套 `chemaster doctor` 一行环境审计、跨平台桌面通知（`chemaster.notify`）、`AGENTS.md ↔ CLAUDE.md` 双 schema 项目级指令兼容等工程化整理，降低了不同技术背景研究者的使用门槛。

4. **系统层工程指标的直接实测**：定义并实测五项指标——**操作性故障自动恢复率 84%**（25 次故障注入）、**应答率与工具调用正确性 98.0%**（5 类任务 × 20 条中英文双语自然语言表述 = 100 测试，agent_ok 100%）、**压力测试 100% 路由正确**（10 分子 × 多语 phrasing = 334 测试）、**大规模调用稳定性**（N=2000 次重复实测，100% 成功率、1 个唯一工具调用序列、wall-clock 均值 131 ms、p99 = 150 ms、滚动均值无漂移）、**MCP 协议跨客户端合规性**（4/4 server 通过完整 initialize → list_tools → call_tool 链路，包括 agent server 本身）。这些指标**完全由系统本身决定**，与所选后端化学软件无关，因此是本工作真正可被严格比较和复现的贡献。新增的应答率与稳定性两项直接回应了导师评审反馈"重点不在准确率而在应答率"以及"如何在上千上万次调用下保持稳定"的诉求。

5. **个人知识库与可覆盖的方法选择规则**：研究者可在 `~/.chemaster/user_kb/` 上传自定义 skill、规则与软件偏好（`prefs.yaml`）；进一步地，**"什么任务用什么方法"的选择规则被显式声明在 `chemaster/kb/rules/method_selection.yaml` 中**（11 条内置规则），用户可在 `~/.chemaster/user_kb/rules/method_selection.yaml` 中按 id 覆盖任意默认规则，命中规则的 id 与 rationale 显式回显在 L2 推荐卡片上。该机制弥补了通用大模型在特定分子家族上的领域盲区，同时让研究者长期积累的工具偏好（如"光谱用 MOMAP、SOC 用 BDF"）能被系统持续遵循而无需每次任务都重新声明。
...
**后端兼容性旁证**：在 S22（全集 22 个体系）、QUEST（10 个分子共 20 个激发态）、蒽（X2C 三阶段相对论）三类公开基准数据集上把 psi4 完整跑通，并补做 11 个 hard-case（重元素、开壳层、较大分子、带电 species）真跑，**11/11 在 HF/sto-3g 与 B3LYP-D3(BJ)/def2-SVP 两套方法下全部成功**。化学层精度的具体数字（S22 MAE 0.245 kcal/mol、QUEST MAE 0.64 eV、蒽 SOC 修正 −0.10 meV）均落在所选方法的内禀误差范围内——这证明的是 ChemMaster 把后端稳定可靠地驱动起来了，**而不是**让计算更准；后者在物理上不可能。
...
这一机制由 `chemaster/agent/user_kb.py` 与 `chemaster/mcp/kb/server.py::_load_user_docs()` 共同实现，并由 25 个单元测试覆盖（涵盖路径解析、偏好加载、文档增删、`kb_search` 集成）。代码层面引入的复杂度可控（约 350 行），但**让系统具备了"随研究者经验持续生长"的可扩展性**——这正是普通商业云方案与开源 Agent 相比的关键差距之一。
...
| 体系 | 类别 | 本工作 (kcal/mol) | 参考值 (kcal/mol) | 误差 (kcal/mol) |
|---|---|---|---|---|
| ammonia_dimer | 氢键 | −3.41 | −3.17 | −0.24 |
...
| benzene_methane | 色散 | −1.43 | −1.45 | +0.02 |
...
- **§4.3.2 应答率与工具调用正确性**（新增，本轮实测，100 tests，98.0%）
- **§4.3.3 大规模调用稳定性**（新增，本轮实测，N=2000，100%）
...
为保证可复现性，本指标使用一个**确定性 MockLLM** —— 它根据用户意图的关键词（中英文双语词表）路由到预期工具，psi4 作为真实后端实际执行计算（即 LLM 层是 mock，但化学层是真跑）。这等价于"系统在面对中英双语、多种自然语言表述时，能否稳定路由到正确工具并完成执行"。结果如表 4.5。

| 任务组 | 测试 phrasings 数 (EN+ZH) | agent 正常完成 | 路由正确 | 应答率/正确率 |
|---|---|---|---|---|
| 能量计算 (energy) | 20 (10+10) | 20 | 20 | **100%** |
| 物理常数 (constant) | 20 (10+10) | 20 | 20 | **100%** |
| 知识库检索 (kb) | 20 (10+10) | 20 | 18 | **90%** |
| 几何优化 (optimize) | 20 (10+10) | 20 | 20 | **100%** |
| Skill 调用 (skill) | 20 (10+10) | 20 | 20 | **100%** |
| **合计** | **100 (50+50)** | **100** | **98** | **98.0%** |
...
100 条自然语言表述中 ChemMaster 全部正常完成 tool-use 循环（agent_ok 率 100%），其中 98 条命中预期工具（路由正确率 98.0%）。两条未命中的情形均出现在 kb 组的查询型表述上（英文 `"Look up basis set rules"` 与对应中文 `"查基组规则"`），反映的是 mock 关键词路由对偏宽泛检索指令的天然极限；在真实大模型路由下（配置 ANTHROPIC_API_KEY 后）该数字预期接近 100%。即使在严苛的 mock 双语路由下，98.0% 的应答正确率仍证明：**ChemMaster 的工具发现、参数构造、tool-use 循环与 trajectory 持久化等环节在多样的中英文自然语言输入下均稳定可用**，导师所关心的"软件能否合规、合理地执行指令"得到了直接验证。可视化见图 4.5。

![Response rate by task group (EN/ZH)](figures/fig_response_rate.png)

*图 4.5 — 应答率与工具调用正确性分组柱状图：5 类 anchor 任务各以 10 条英文和 10 条中文自然语言表述驱动，图中标注实际命中预期工具的比例（带计数标签）；100 次测试中应答率 100%，路由正确率 98.0%*

### 4.3.3 大规模调用稳定性（新增，本轮实测）

**指标定义**：把同一个 anchor 任务（"Compute the energy of H2 using HF/sto-3g"）通过 ChemAgent 跑 N=2000 次，统计：

- *success_rate* —— 全部 2000 次中无异常退出的比例；
- *unique_tool_sequences* —— trajectory 里非 builtin 工具调用序列的去重哈希数，理想值为 1（即每次都给出相同的工具调用序列）；
- *wall-clock 分布* —— 均值、标准差、p50 / p95 / p99 / max；
- *漂移* —— 每次调用耗时对调用序号的回归与滚动均值，理想值为水平直线。
...
| 成功率 | **100.00%** (0 失败) | ≥ 99% | ✓ |
| 唯一工具调用序列哈希数 | **1** | ≤ 1 | ✓ |
| wall-clock 均值 | 131 ms | — | — |
| wall-clock 标准差 | 4.9 ms | — | — |
| wall-clock 中位数 (p50) | 130 ms | — | — |
| wall-clock p95 | **142 ms** | — | — |
| wall-clock p99 | 150 ms | — | — |
| wall-clock 最大值 | ≈ 170 ms | — | — |

*表 4.6 — 大规模调用稳定性（数据来自 `benchmarks/engineering_metrics/scalability.json`）*

2000 次重复调用下：(i) **零失败**——0/2000 没有任何异常退出；(ii) **完全确定**——2000 次的工具调用序列哈希全部一致，证明 trajectory 与中间状态可复现；(iii) **时间分布极窄**——标准差仅 4.9 ms，p99 比 p50 只高 20 ms，分布几乎是一条线；(iv) **无漂移**——耗时对调用序号的散点 + 50 点滚动均值（图 4.6 右）是水平的，说明系统行为不随重复次数发生累积或退化。这一结果**直接回应了导师反馈第 3 条**："问一万次'计算水的能量'每一次都能正确执行"——本指标在 2000 次重复下尚未出现任何失败、且滚动均值无漂移，因此外推到 10000 次也无理论上的失败累积来源（LLM 层用 mock 排除了模型变异性；剩余的失败概率仅来自后端工具与系统调用本身，psi4 在小分子 SCF 上的稳定性是已知的）。可视化见图 4.6。

![Scalability: histogram and time-series](figures/fig_scalability.png)

*图 4.6 — 大规模调用稳定性 (N = 2000 次重复同一 anchor 任务)。左：耗时直方图，分布极窄 (mean = 131 ms, std = 4.9 ms, p95 = 142 ms, p99 = 150 ms)；右：耗时对调用序号的散点 + 50 点滚动均值，水平直线表示系统行为不随重复次数发生漂移或累积*
...
4. **系统层工程指标的直接实测（本工作真正的可比较贡献）** —— 按导师反馈把重点从"准确率"转向"应答率 + 执行稳定性"，完成了五项指标的直接实测：(i) **操作性故障自动恢复率 84%**（25 次故障注入），(ii) **应答率与工具调用正确性 98.0%**（5 类任务 × 20 条中英文表述 = 100 测试，分组：能量 / 常数 / 优化 / skill 各 20/20=100%，kb 18/20=90%），(iii) **压力扩展集 100% 路由正确**（10 分子 × 多语 phrasing = 334 测试，agent_ok 100%），(iv) **大规模调用稳定性 100% 成功率 / 1 个唯一序列 / wall-clock 均值 131 ms ± 4.9 ms / p99 = 150 ms**（N=2000 次重复实测，散点 + 50 点滚动均值无漂移），(v) **MCP 协议跨客户端合规性 4/4**（const / kb / calc_psi4 / 整个 agent 内核 `chemaster.mcp.agent.server` 均通过完整 initialize → list_tools → call_tool 链路）。**这五项指标完全由系统本身决定，与所选后端化学软件无关**，因此是本工作真正可被严格比较和复现的贡献——它们共同回答了导师反馈"如何保证大规模推广下的稳定性"以及"软件能否合规、合理地执行指令"的问题。其余三项（提交摩擦时间节省、推荐接受率、trajectory 自主步占比）因依赖真人被试或真实大模型 API 留作后续工作，相关实验协议已固化在 `docs/BENCHMARK_PROTOCOL.md` 中。

5. **后端兼容性旁证** —— 在 S22（基态结合能）、QUEST（垂直激发能）、蒽（X2C SOC）三个公开 benchmark 上完成把后端跑通的验证，按导师反馈把 S22 扩展到全集 22 体系、QUEST 扩展到 10 分子 20 状态；进一步补做 11 个 hard-case（重元素、开壳层、较大分子、带电 species）真跑，**11/11 在 HF/sto-3g 与 B3LYP-D3(BJ)/def2-SVP 两套方法下全部成功**。化学层实测：S22 全 22 体系的 B3LYP-D3(BJ)/def2-TZVP+counterpoise MAE 0.245 kcal/mol，落在该方法在 S22 上的文献常规精度区间；QUEST 10 分子 20 状态的 TD-CAM-B3LYP/def2-SVP 总体 MAE 0.64 eV（价层态平均 MAE 0.45 eV、Rydberg 态平均 MAE 0.91 eV，与 def2-SVP 缺乏弥散函数的方法学预期一致）；蒽 X2C 标量修正 −5.28 eV / SOC 修正 −0.10 meV——三组数据均落在所用方法的内禀误差范围内。**这些精度数字证明的是 ChemMaster 把后端稳定可靠地驱动起来了**——任何不调用 ChemMaster、用同样方法+基组的脚本，得到的 MAE 不会变；**ChemMaster 不会让 B3LYP-D3 算得比 B3LYP-D3 自身准**，后者在物理上不可能。蒽完整 BDF + MOMAP TVCF 流水线留作未来工作；同时新增 `chem.calc_pyscf` MCP server，以开源 PySCF 2.13 真实运行了蒽的三阶段 X2C 相对论计算作为 BDF SOC 路径的开源 reference。

6. **新增个人知识库（user_kb）机制**回应导师反馈"大模型领域盲区 + 个性化偏好"问题：研究者可在 `~/.chemaster/user_kb/` 上传自定义 skill / 规则 / `prefs.yaml`（如"SOC 一律用 BDF、光谱用 MOMAP"），系统启动时合并加载并影响 Agent 推荐决策。机制由约 350 行代码 + 25 个单元测试实现（仓库总测试数从 256 提升至 281，其中 278 通过、3 个因测试机器未配置 LLM API key 而跳过）。
