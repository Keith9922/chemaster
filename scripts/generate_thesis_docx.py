...
def write_chapter1(doc):
    ...
    add_body(doc,
        "本文的主要工作包括：(1) 系统的整体设计与基于五层架构的实现；(2) 在"
        "Agent 内核中提出「承担操作性工作、保留化学决策权」"
        "的设计原则与三级权限分级机制；(3) 命令行（CLI）、终端交互界面"
        "（Textual TUI）与本地 Web 三种前端的实现与启动验证；(4) 在 S22 与"
        "QUEST 两个公开基准上的真实计算验证；(5) 对操作性故障自动恢复率指标"
        "的真实测量；(6) MCP 协议合规性的独立验证。本文未完成的工作包括：依"
        "赖 BDF 与 MOMAP 的蒽分子速率与动力学验证；需要真人被试参与的提交时"
        "间节省率与化学决策推荐接受率；以及需要真实大模型 API 的运行轨迹自"
        "主步占比指标。本文在第 4 章相应位置如实标注上述未完成项及其原因。")
    add_body(doc,
...
def write_chapter3(doc):
    ...
    add_figure(doc, FIGS_V4 / "fig_permission.png",
               "图 3.1   操作性工作 vs 化学决策的权限分级机制（设计示意图）",
               width_cm=15.0)
...
def write_chapter4(doc):
    ...
    add_h2(doc, "4.1  实验环境与数据真实性说明")
    add_body(doc,
        "本章数据采集于一台 Apple Silicon Mac mini（macOS Darwin arm64，"
        "48 GB RAM），软件环境为 Python 3.13 + psi4 1.10 + textual 8.2 +"
        "FastAPI 0.135 + Playwright 1.59。受测试机器软件许可条件所限，"
        "Gaussian、BDF、MOMAP 三个商业 / 学术许可软件未能在本机安装。")
    add_body(doc,
        f"复现说明：本工作的全部源代码、Benchmark 数据、可复现脚本与运行"
        f"轨迹均已开源，发布于 {GITHUB_URL} 。完整运行本章实验需同时具备"
        f" psi4、pint、rdkit、ase 四个 Python 依赖。其中 psi4 仅可通过"
        f" Conda 安装，其余三者可通过 pip 安装。本工作的命令在配置完整依"
        f"赖的 Conda 环境（路径 /opt/miniconda3/bin/python）中运行。本章"
        f"涉及命令的标准调用形如：/opt/miniconda3/bin/python -m pytest"
        f" tests/unit/ -q 与 /opt/miniconda3/bin/python scripts/"
        f"benchmarks/run_s22_psi4.py。")
    add_body(doc,
        "出于诚实标注实验范围的考虑，本章在每节明确说明所述数据是真实采集"
        "还是占位（待真实软件接入后补充）。具体而言：4.2 节 S22 基准与 4.3"
        "节 QUEST 基准均通过 psi4 实跑得到真实数据；4.4.1 节技术性故障自动"
        "恢复率通过故障注入实测得到；其余三项工程指标依赖真人被试或真实大"
        "模型 API，本文未完成数据采集，作为占位说明列出对应实验协议供后续"
        "工作执行。")
    ...
    add_figure(doc, FIGS_REAL / "fig_real_engineering.png",
               ...
               "终端屏幕截图。其中操作性故障自动恢复为本工作完成的指标"
               "（21/25, 84%, 通过 80% 目标），运行轨迹自主步占比因 mock "
               "LLM 未触发工具调用而 skipped，其余两项工程指标以 "
               "not_collected 状态记录。",
               width_cm=15.5)
    ...
    add_h3(doc, "4.4.1  技术性故障自动恢复率（已完成）")
    add_body(doc,
    ...
    add_h3(doc, "4.4.2  其他工程指标（未完成）")
    add_body(doc,
        "本工作原计划采集的其他三项工程指标因实验条件限制未在本文中完成数"
        "据收集，列出如下，对应实验协议详见仓库 docs/BENCHMARK_PROTOCOL.md：")
    add_body(doc,
        "(1) 任务提交时间节省率：需要至少 2 名熟悉量子化学软件的被试在"
        "「无系统辅助」与「使用 ChemMaster」两种模式下完成同一组任务并自报或"
        "自动记录耗时。受答辩前时间所限，被试招募与实验执行未能完成。")
    add_body(doc,
        "(2) 化学决策推荐接受率：需要被试在 anchor 任务上响应 recommend 卡"
        "片（接受 / 修改 / 取消），统计接受比例。原因同上。recommend 机制"
        "本身在系统中已实现并由单元测试覆盖，但接受率必须由真人决定，无法"
        "通过模拟方式得到有意义的数据。")
    add_body(doc,
        "(3) 运行轨迹自主步占比：需在真实大模型 API 环境下运行 anchor 任"
        "务，对运行轨迹中 decision_authority 字段做统计。decision_authority"
        "标签的写入逻辑已实现并经单元测试验证，但本工作环境中无可用的大模"
        "型 API 密钥，无法触发包含 recommend 调用与多种工具组合的真实运行。")
    ...
    add_figure(doc, FIGS_V4 / "fig_comparison.png",
               "图 4.10   ChemMaster 与同类工作在 8 个维度上的对比（设计示意图）",
               width_cm=15.5)
    add_body(doc,
...
def write_conclusion(doc):
    ...
    add_body(doc,
        "(5) 测得 5 类操作性故障的自动恢复率为 84%，达到设定目标。")
    add_body(doc,
        "本文的局限主要包括：(1) 受测试机器软件许可条件所限，BDF 与 MOMAP"
        "未能在本工作中真实接入，相应的蒽分子速率与动力学验证留作未来工"
        "作；(2) 三项需要真人被试参与或真实大模型 API 的工程指标（提交时"
        "间节省率、推荐接受率、轨迹自主步占比）未在本工作中完成数据采集；"
        "(3) 本文的几何模型对部分 S22 体系采用了基于文献描述构建的近似坐"
        "标，导致这些体系的误差大于使用完整 S22A 数据集时的常规水平。")
    add_body(doc,
        "未来工作的主要方向包括：在装有 BDF 与 MOMAP 的环境中完成蒽分子的"
        "完整速率与动力学验证；招募真人被试完成提交时间节省率与推荐接受率"
        "的实验测量；将系统部署到真实大模型 API 环境下完成轨迹自主步占比"
        "的统计；在更多公开基准（如 GMTKN55、TADF 文献体系）上扩展验证；"
        "以及在实际科研课题中应用并迭代改进推荐机制的化学判断质量。")
